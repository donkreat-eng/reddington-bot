...
def fetch_crypto_price(ticker: str) -> dict:
    ...
    coin_id = {"BTC": "bitcoin", "ETH": "ethereum", "BNB": "binancecoin",
               "SOL": "solana", "XRP": "ripple"}[ticker]
...
def fetch_crypto_change(ticker: str) -> dict:
    """24h change % + volume from Binance klines (most reliable)."""
    symbol = BINANCE_SYMBOL[ticker]
    data = _http_json(f"{BINANCE_BASE}/api/v3/ticker/24hr", {"symbol": symbol})
    return {
...
def fetch_crypto_ohlc(ticker: str, days: int = 7) -> list:
    """4h klines OHLC for chart."""
    symbol = BINANCE_SYMBOL[ticker]
    interval = "4h"
    limit = days * 6 if days == 7 else days * 24
...
def fetch_crypto_positional(ticker: str) -> dict:
    """OI, funding, L/S ratio from Binance futures."""
    symbol = BINANCE_SYMBOL[ticker]
    out = {}
    try:
        oi = _http_json(f"{BINANCE_BASE}/futures/data/openInterestHist",
    ...
    try:
        fr = _http_json(f"{BINANCE_BASE}/fapi/v1/premiumIndex", {"symbol": symbol})
        out["funding"] = float(fr["lastFundingRate"]) * 100
    except Exception:
        out["funding"] = None

    try:
        ls = _http_json(f"{BINANCE_BASE}/futures/data/globalLongShortAccountRatio",
                        {"symbol": symbol, "period": "1h", "limit": 1})
