...
def _http_json(url, params=None, timeout=10):
...
def fetch_crypto_price(ticker):
    ...
    return {"ticker": ticker, "price": median, "spread_pct": spread_pct,
            "n_sources": len(results), "sources": results}
...
def _okx_oi(inst_id, mark_price):
...
def _okx_extras(symbol):
    ...
    return {"change_24h": change_pct, "turnover_24h": turnover_usdt,
            "funding": funding, "oi_usdt": oi_usdt}


def fetch_crypto_change(ticker):
    symbol = BINANCE_SYMBOL[ticker]
    try:
        data = _http_json(f"{BINANCE_BASE}/api/v3/ticker/24hr", {"symbol": symbol})
        return {"change_24h": float(data["priceChangePercent"]),
                "volume_24h": float(data["quoteVolume"]), "source": "binance"}
    except Exception as e:
        log("market_overview", f"change {ticker} Binance FAIL: {e}; trying Bybit")
    try:
        ex = _bybit_extras(symbol)
        return {"change_24h": ex["change_24h"], "volume_24h": ex["turnover_24h"],
                "source": "bybit"}
    except Exception as e:
    ...
    try:
        data = _http_json(f"{COINGECKO_BASE}/coins/{COINGECKO_IDS[ticker]}",
        ...
        return {"change_24h": float(change), "volume_24h": float(vol), "source": "coingecko"}
    except Exception as e2:
        log("market_overview", f"change {ticker} CoinGecko FAIL: {e2}")
        return {"change_24h": 0.0, "volume_24h": 0.0, "source": "none"}


def _kraken_ohlc(symbol, minutes=240, days=7):
    pair = {"BTCUSDT": "XBTUSDT", "ETHUSDT": "ETHUSDT", "BNBUSDT": "BNBUSDT",
            "SOLUSDT": "SOLUSDT", "XRPUSDT": "XRPUSDT"}[symbol]
    ...
    for c in candles:
        if c[0] >= cutoff:
            out.append((c[0] * 1000, float(c[1]), float(c[2]), float(c[3]), float(c[4])))
    return out


def _coingecko_ohlc(coin_id, days=7):
    data = _http_json(f"{COINGECKO_BASE}/coins/{coin_id}/ohlc",
                      {"vs_currency": "usd", "days": days})
    return [(c[0], float(c[1]), float(c[2]), float(c[3]), float(c[4])) for c in data]


def fetch_crypto_ohlc(ticker, days=7):
    symbol = BINANCE_SYMBOL[ticker]
    limit = days * 6
    try:
        ...
        return [(int(t[0]), float(t[1]), float(t[2]), float(t[3]), float(t[4])) for t in raw]
    except Exception as e:
...
def fetch_crypto_positional(ticker):
    symbol = BINANCE_SYMBOL[ticker]
    out = {}
    def _bin(path, params, timeout=10):
        return _http_json(f"{BINANCE_BASE}{path}", params, timeout=timeout)
    try:
        oi = _bin("/futures/data/openInterestHist", {"symbol": symbol, "period": "1h", "limit": 1})
        out["oi_usdt"] = float(oi[0]["sumOpenInterestValue"]) if oi else 0.0
    except Exception as e:
        log("market_overview", f"OI {ticker} Binance FAIL: {e}; trying Bybit")
        try:
            ex = _bybit_extras(symbol)
            out["oi_usdt"] = ex["oi_usdt"]
        except Exception as e2:
            log("market_overview", f"OI {ticker} Bybit FAIL: {e2}; trying OKX")
            try:
                ex = _okx_extras(symbol)
                out["oi_usdt"] = ex["oi_usdt"]
    ...
    try:
        ...
        log("market_overview", f"funding {ticker} Binance FAIL: {e}; trying Bybit")
        try:
            ex = _bybit_extras(symbol)
            out["funding"] = ex["funding"]
        except Exception as e2:
            log("market_overview", f"funding {ticker} Bybit FAIL: {e2}; trying OKX")
            try:
                ex = _okx_extras(symbol)
                out["funding"] = ex["funding"]
            except Exception as e3:
                ...
                out["funding"] = None
    try:
        ls = _bin("/futures/data/globalLongShortAccountRatio", {"symbol": symbol, "period": "1h", "limit": 1})
...
def build_top5_chart(ohlc_by_ticker, out_path):
...
def build_caption(prices, changes, ts):
...
def build_body(prices, changes, positional, fng_value, ts):
...
def main():
    ...
    prices["_btc_price"] = prices["BTC"]["price"]
    chart_path = post_dir() / f"top5_btc_{ts.strftime('%Y%m%d_%H%M')}.png"
    build_top5_chart({"BTC": btc_ohlc, "_btc_price": prices["BTC"]["price"]}, chart_path)
